...
def test_random_sequence_valid_invalid_inputs(nb_of_runs, char_min_size, char_max_size):
    ...
    for j in range(1, nb_of_runs):
        ...
        if VALID_INVALID_PICK == "invalid":
            xbridge_utils.generate_new_set_of_data(True)
        else:
            xbridge_utils.generate_new_set_of_data(False)
...
def test_defined_order_sequence(nb_of_runs, invalid_data_bool):
    time_distribution = []
    total_elapsed_seconds = 0
    elapsed_Time = 0
    for i in range(1, nb_of_runs):
        if invalid_data_bool:
            xbridge_utils.generate_new_set_of_data(True)
        else:
            xbridge_utils.generate_new_set_of_data(False)
