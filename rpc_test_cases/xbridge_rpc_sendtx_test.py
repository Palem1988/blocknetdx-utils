...
class signUnitTest(unittest.TestCase):
    ...
    def test_invalid_cancel_1(self):
        try:
            self.assertIsInstance(xbridge_rpc.send_tx(" "), dict)
            self.assertIsInstance(xbridge_rpc.send_tx(""), dict)
            self.assertIsInstance(xbridge_rpc.send_tx("[]"), dict)
            self.assertIsInstance(xbridge_rpc.send_tx("[[]]"), dict)
            self.assertIsInstance(xbridge_rpc.send_tx("{{}}"), dict)
            ...
            self.assertIsInstance(xbridge_rpc.send_tx("''"), dict)
            self.assertIsInstance(xbridge_rpc.send_tx("'"), dict)
            # print("dxSend Unit Test Group 1 OK")
        except AssertionError as e:
            # print("****** dxSend Unit Test Group 1 FAILED ******")
            xbridge_logger.logger.info('dxSend unit test group 1 FAILED')
            ...
            for sub_item in itm:
                with self.subTest(sub_item=sub_item):
                    ...
                    try:
                        ...
                        self.assertIsInstance(xbridge_rpc.send_tx(generated_str), dict)
                        run_count += 1
                    except AssertionError as e:
                        # print("****** dxSend Unit SUBTEST 2 FAILED ON PARAMETER: %s ******" % generated_str)
                        xbridge_logger.logger.info('dxSend unit test group 2 FAILED on parameter: %s', generated_str)
                        run_count += 1
            ...
            for sub_item in itm:
                with self.subTest(sub_item=sub_item):
                    ...
                    try:
                        ...
                        # print("****** dxSend Unit SUBTEST 3 FAILED ON PARAMETER: %s ******" % generated_str)
                        xbridge_logger.logger.info('dxSend unit test group 3 FAILED on parameter: %s', generated_str)
                        run_count += 1
            ...
            for sub_item in itm:
                with self.subTest(sub_item=sub_item):
                    ...
                    try:
                        ...
                        # print("****** dxSend Unit SUBTEST 4 FAILED ON PARAMETER: %s ******" % generated_str)
                        xbridge_logger.logger.info('dxSend unit test group 4 FAILED on parameter: %s', generated_str)
                        run_count += 1
